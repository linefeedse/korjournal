...
class Invoice(models.Model):
    ...
    def renderpdf(self, fileobj):
        tzsweden = tz.gettz("Europe/Stockholm")
        locale.setlocale(locale.LC_NUMERIC, "sv_SE.utf8")

        invoicedate = self.invoicedate.strftime('%Y-%m-%d')
        duedate = self.duedate.strftime('%Y-%m-%d')
        customer_designation = self.customer.username
        customer_name = self.customer_name
        customer_address = self.customer_address
        specification = self.specification
        scope = "%s - %s" % (self.scope_from.strftime('%Y-%m-%d'),
            self.scope_to.strftime('%Y-%m-%d'))
        amount = self.amount
        vat_percent = 25
